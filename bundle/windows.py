...
class AlibabaCrawler:
    ...
    def get_chrome_driver(self):
        ...
        if os.name == 'nt':  # Windows
            ...
            subprocess.call(["pkill", "chrome"])  # For Linux/Mac, use pkill to kill Chrome
...
class InteractTickets:
    def __init__(self, scrapy, driver, departure_times: list = (), order=1):
        self.scrapy = scrapy
        self.driver = driver
...
def get_inputs_from_user():
    ...
    while True:
        ...
        if not re.match(date_pattern, train_date):
            print(Fore.RED + "❌ Invalid date format. Use the format 12-12-1403.")
            continue

        time_pattern = r'^(?:[01]?\d|2[0-3]):[0-5]\d$'
        train_time = input(
            Fore.BLUE + '⏰ Enter departure time (15:00) or leave empty (comma-separated for multiple times): ').strip()
        if train_time:
        ...
        return username, train_date, times if train_time else [], qtot, order


def search_for_ticket(username, t_date, t_time, QToT, order):
    ...
    while True:
        ...
        interactor = InteractTickets(driver=driver, scrapy=scrapy, departure_times=t_time, order=order)
...
if __name__ == '__main__':
    username, t_date, t_time, qtot, order = get_inputs_from_user()
    ...
    search_for_ticket(username=username, t_date=t_date, t_time=t_time, QToT=qtot, order=order)
